...
func GetsimilarrecipesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/recipes/%s/similar%s", cfg.BaseURL, id, queryString)
...
func CreateGetsimilarrecipesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_recipes_id_similar",
		mcp.WithDescription("Get Similar Recipes"),
		mcp.WithString("id", mcp.Required(), mcp.Description("The id of the source recipe for which similar recipes should be found.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetsimilarrecipesHandler(cfg),
	}
}
